...
NAMES = ('addAdCampaign','editAdCampaign','removeAdCampaign','addAdSet','removeAdSet','addCreative','removeCreative')


def fail(message: str) -> None:
    raise SystemExit('AD_STRUCTURE_METHOD_DIAGNOSTIC_FAILED: ' + message)
...
for path in files:
    ...
    for name in NAMES:
        ...
        found[name]+=1;start,end=bounds;source=text[start:end].strip()
        print(f'AD_STRUCTURE_METHOD_SOURCE::{name}::{base64.b64encode(source.encode()).decode()}')
for name,count in found.items():
    if count!=1: fail(f'{name} expected exactly once, found {count}')
print('AD_STRUCTURE_METHOD_DIAGNOSTIC_OK: methods=7; encoding=base64')
