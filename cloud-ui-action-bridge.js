(()=>{
  ...
  const finalizeClientListNavigation=()=>{
    try{vm.navigateTo?.('clients')}catch{}
    vm.currentPage='clients';
    vm.mobileMenuOpen=false;
    setClientListUrl();
    try{vm.$forceUpdate?.()}catch{}
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      if(vm.currentPage!=='clients')vm.currentPage='clients';
      try{vm.$forceUpdate?.()}catch{}
    }));
  };
  ...
  install();
  window.__GROWTHOPS_UI_ACTION_BRIDGE__={installed:true,version:'native-action-bridge-v7'};
})();
