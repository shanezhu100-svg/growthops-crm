...
# This finalizer deliberately runs before inline-script extraction and Vue template
# precompilation. Both the template and the embedded application object are patched
# in one source-of-truth HTML file; later build stages externalize/compile that result.
...
html = replace_once(html, normalize_anchor, normalize_new, 'normalizeClient collection plan')
...
html = replace_once(html, default_anchor, default_new, 'defaultForm collection plan')

INDEX.write_text(html, encoding='utf-8')
sha = hashlib.sha256(html.encode('utf-8')).hexdigest()
print(
    'COLLECTION_PLAN_UI_FINALIZE_OK: '
    'stage=pre-inline-extraction+pre-vue-precompile; client-form=monthly+semi-monthly; '
    'semi-monthly=first-day+ratio+automatic-month-end; legacy-client=normalized-monthly-default; '
