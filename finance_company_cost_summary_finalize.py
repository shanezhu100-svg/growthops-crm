...
# Vue templates are already compiled by this stage. Locate only the one explanatory
# sentence whose core claims customer-specific + company-project + company-public
# cost are combined, then normalize that whole sentence to the reviewed company-only
# wording. Other CLIENT labels/detail text remain untouched.
...
sentence_re = re.compile(r'已包含客户专属成本 \+ 公司项目成本 \+ 公司公共成本[^。]*。')
matches = sentence_re.findall(registry)
if len(matches) != 1:
    fail(f'old summary sentence expected exactly once, found {len(matches)}')
if NEW_COPY in registry:
    fail('new summary copy already present before correction')
registry = sentence_re.sub(NEW_COPY, registry, count=1)
if OLD_COPY_CORE in registry or registry.count(NEW_COPY) != 1:
