...
alertList(){const legacyAlertList=this.$options?.methods?._legacyAlertList||this._legacyAlertList;if(typeof legacyAlertList!=='function')return[];const base=legacyAlertList.call(this),receivables=Array.isArray(this.financeReceivables)?this.financeReceivables:[],explicit=receivables.map(r=>({r,nodes:this.financeReceivableCollectionNodes(r)})).filter(x=>x.nodes.length);if(!explicit.length)return base;const masterAlertIds=new Set(explicit.map(x=>`RECEIVABLE-${x.r.id}`)),out=base.filter(item=>!masterAlertIds.has(String(item?.id||''))),legacyRows=receivables.filter(r=>!explicit.some(x=>x.r===r)),synthetic=[];for(const entry of explicit){const allocation=this.financeReceivableCollectionAllocation(entry.r);for(const node of entry.nodes){const paid=Number(allocation[node.id]||0);synthetic.push({...entry.r,id:`${entry.r.id}::COLLECTION::${node.id}`,amount:node.amount,dueDate:node.dueDate,payments:paid>0?[{id:`allocation-${node.id}`,amount:paid}]:[],collectionNodes:[]})}}const view=Object.create(this);view.financeReceivables=[...legacyRows,...synthetic];const nodeAlerts=legacyAlertList.call(view),prefixes=explicit.map(x=>`RECEIVABLE-${x.r.id}::COLLECTION::`);for(const item of nodeAlerts){const id=String(item?.id||'');if(prefixes.some(prefix=>id.startsWith(prefix)))out.push(item)}return out}'''

create_wrapper = r'''createReceivableForClientMonth(client,month,opts){const legacyCreate=this.$options?.methods?._legacyCreateReceivableForClientMonth||this._legacyCreateReceivableForClientMonth;if(typeof legacyCreate!=='function')return 0;const before=new Set(Array.isArray(this.financeReceivables)?this.financeReceivables:[]),added=legacyCreate.call(this,client,month,opts);if(!added||!client||String(client.collectionPlan?.mode||'').toUpperCase()!=='SEMI_MONTHLY')return added;const created=(Array.isArray(this.financeReceivables)?this.financeReceivables:[]).filter(row=>!before.has(row)&&String(row?.clientId)===String(client.id));for(const row of created){const nodes=this.financeReceivableBuildCollectionNodes(row,client.collectionPlan);if(nodes.length)row.collectionNodes=nodes}return added}'''
...
print(
    ...
    'vue-runtime=legacy-delegates-from-options+vm-fallback; '
