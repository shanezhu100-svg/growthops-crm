-- Read-only post-change verification for P5 Group 3.
with functions as (
  select p.oid,
         p.proname,
  ...
  where (proname='crm_list_users' and identity_args='p_token text')
     or (proname='crm_upsert_user' and identity_args='p_token text, p_user_id uuid, p_name text, p_username text, p_password text, p_role text, p_enabled boolean')
     or (proname='crm_delete_user' and identity_args='p_token text, p_user_id uuid')
)
select
  proname,
  identity_args,
