-- Read-only post-change verification for P5 Group 5.
with functions as (
  select p.oid,
         p.proname,
  ...
  where (proname='crm_load_state_v3' and identity_args='p_token text')
     or (proname='crm_save_state' and identity_args='p_token text, p_state jsonb, p_expected_revision bigint')
     or (proname='crm_logout' and identity_args='p_token text')
)
select
  proname,
  identity_args,
