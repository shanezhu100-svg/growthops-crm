-- Read-only post-change verification for P5 Group 6.
with functions as (
  select p.oid,
         p.proname,
  ...
  where (proname='crm_login_v3' and identity_args='p_username text, p_password text')
     or (proname='crm_public_status' and identity_args='')
)
select
  proname,
  identity_args,
