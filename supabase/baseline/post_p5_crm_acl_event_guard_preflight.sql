-- READ-ONLY preflight for the Post-P5 CRM ACL event guard.
with inventory as (
...
select
  ...
  exists(select 1 from pg_event_trigger where evtname='growthops_crm_acl_guard_ddl') as guard_event_exists,
  to_regprocedure('public.growthops_crm_acl_guard_ddl()') is not null as guard_function_exists,
