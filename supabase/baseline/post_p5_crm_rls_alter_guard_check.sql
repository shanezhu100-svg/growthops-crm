-- READ-ONLY post-check for the Post-P5 CRM ALTER-table RLS guard.
with f as (
  ...
  select e.evtname,e.evtenabled,e.evttags,pg_get_userbyid(e.evtowner) event_owner,
         p.prosecdef,p.proconfig,pg_get_userbyid(p.proowner) function_owner,
  ...
  where e.evtname='growthops_crm_rls_guard_ddl'
), inventory as (
...
select
  ...
  (select evtname from guard) alter_rls_guard_event,
  (select evtenabled from guard) alter_rls_guard_enabled,
  (select evttags from guard) alter_rls_guard_tags,
  (select event_owner from guard) alter_rls_guard_event_owner,
  (select function_owner from guard) alter_rls_guard_function_owner,
  (select prosecdef from guard) alter_rls_guard_security_definer,
  (select proconfig from guard) alter_rls_guard_config,
  (select anon_exec from guard) alter_rls_guard_anon_exec,
  (select authenticated_exec from guard) alter_rls_guard_authenticated_exec,
  (select service_exec from guard) alter_rls_guard_service_exec,
