-- READ-ONLY post-check for trusted login source-bucket propagation.
with login_fn as (
...
select
  (select owner='postgres' from login_fn) as owner_ok,
  (select prosecdef from login_fn) as security_definer_ok,
  (select proconfig @> array['search_path=public, extensions'] from login_fn) as search_path_ok,
  (select position('x-growthops-source-bucket' in def)>0 from login_fn) as custom_bucket_present,
  (select position('^[0-9a-f]{24}$' in def)>0 from login_fn) as strict_bucket_shape_present,
  (select position('x-forwarded-for' in def)>0 and position('cf-connecting-ip' in def)>0 from login_fn) as compatibility_fallback_present,
