-- READ ONLY: preflight for service_role direct CRM relation ACL removal.
with crm_tables as (
...
select
  ...
  (select count(distinct table_name) from information_schema.role_table_grants where table_schema='public' and table_name like 'crm_%' and grantee='service_role')::int service_table_count,
  has_sequence_privilege('service_role','public.crm_server_audit_logs_id_seq','SELECT') service_sequence_select,
  has_sequence_privilege('service_role','public.crm_server_audit_logs_id_seq','UPDATE') service_sequence_update,
  has_sequence_privilege('service_role','public.crm_server_audit_logs_id_seq','USAGE') service_sequence_usage,
