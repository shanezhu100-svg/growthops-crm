-- READ-ONLY post-check for post-P5 service_role CRM RPC minimization.
-- Expected after apply:
--   functions=40, PUBLIC/anon/authenticated/service_role=0/0/0/12
--   preserved service_role entries=12, unexpected service_role entries=0
--   RLS=9/9, sequence browser ACL unchanged
...
select
  ...
  (select count(*) from funcs f left join allowed a on a.name=f.proname where a.name is null and f.service_exec) as unexpected_service_exec,
