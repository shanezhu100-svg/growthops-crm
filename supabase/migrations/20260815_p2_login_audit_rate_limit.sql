-- GrowthOps CRM P2: failed-login audit + conservative source throttling.
-- Staged with the application change first. Apply only after the compatible
-- cloud-adapter login path is live, because failed attempts return a JSON error
-- so their audit rows can commit instead of being rolled back by an exception.
--
-- Security properties:
-- - no password, token, 2FA/recovery secret, raw IP, or request-header dump is stored;
-- - the source identifier is a truncated SHA-256 bucket of the source IP;
-- - if the proxy source IP is unavailable, source throttling is disabled rather
--   than degrading into a global lockout key;
-- - the client receives the same INVALID_CREDENTIALS result for bad credentials
--   and throttled attempts.

create index if not exists crm_server_audit_login_pair_idx
on public.crm_server_audit_logs (
  (detail->>'sourceBucket'),
  (detail->>'usernameKey'),
  created_at desc
)
...
create index if not exists crm_server_audit_login_source_idx
on public.crm_server_audit_logs (
  (detail->>'sourceBucket'),
  created_at desc
)
