-- Harden on-demand Vault credential reveal without changing normal CRM session UX.
-- Sensitive reveal requires a fresh ADMIN session and is rate-limited server-side.

create index if not exists crm_server_audit_logs_reveal_user_created_idx
  ...
  if v_session_created is null or v_session_created < now() - interval '12 hours' then
    if not exists (
      ...
      values(
        ...
        c.user_id,
        'REVEAL_CLIENT_SECRETS_REAUTH_REQUIRED',
        jsonb_build_object('clientId',coalesce(p_client_id,''),'reason','SESSION_NOT_FRESH')
      );
    end if;
