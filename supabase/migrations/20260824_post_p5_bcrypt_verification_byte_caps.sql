-- Repository mirror of Production migration 20260824031938 post_p5_bcrypt_verification_byte_caps.
-- Keep every pgcrypto bcrypt verification path inside the 72 UTF-8 byte boundary.
...
BEGIN
  ...
  IF v_bootstrap_md5 <> 'ee2d5b74bb2b5fa3ed8e1b4bb214da84' THEN
    RAISE EXCEPTION 'BCRYPT_VERIFY_CAP_PREFLIGHT_BOOTSTRAP_DRIFT:%', v_bootstrap_md5;
  END IF;
  IF v_login_md5 <> '88eb27fbc0acb799ef5cb63e35f1168e' THEN
    RAISE EXCEPTION 'BCRYPT_VERIFY_CAP_PREFLIGHT_LOGIN_DRIFT:%', v_login_md5;
  END IF;
  IF v_unlock_md5 <> '03dbcf1a878ec88b7e9f71ecc9cac5d3' THEN
    RAISE EXCEPTION 'BCRYPT_VERIFY_CAP_PREFLIGHT_UNLOCK_DRIFT:%', v_unlock_md5;
  END IF;
END;
$preflight$;
...
BEGIN
  IF md5(pg_get_functiondef(v_bootstrap)) <> '108be4243b6cd38522a06da77a2ead7b' THEN
    RAISE EXCEPTION 'BCRYPT_VERIFY_CAP_POSTCHECK_BOOTSTRAP_FINGERPRINT';
  END IF;
  IF md5(pg_get_functiondef(v_login)) <> 'd3af3bfea698eab3b6592da29ef3329a' THEN
    RAISE EXCEPTION 'BCRYPT_VERIFY_CAP_POSTCHECK_LOGIN_FINGERPRINT';
  END IF;
  IF md5(pg_get_functiondef(v_unlock)) <> '0d40dda5b2bc99af44e5c39e5295b513' THEN
    RAISE EXCEPTION 'BCRYPT_VERIFY_CAP_POSTCHECK_UNLOCK_FINGERPRINT';
  END IF;

  IF NOT (SELECT prosecdef FROM pg_proc WHERE oid=v_bootstrap)
     OR NOT (SELECT prosecdef FROM pg_proc WHERE oid=v_login)
     OR NOT (SELECT prosecdef FROM pg_proc WHERE oid=v_unlock) THEN
    RAISE EXCEPTION 'BCRYPT_VERIFY_CAP_POSTCHECK_SECURITY_DEFINER';
  END IF;

  IF (SELECT proconfig FROM pg_proc WHERE oid=v_bootstrap) IS DISTINCT FROM ARRAY['search_path=public, extensions']::text[]
     OR (SELECT proconfig FROM pg_proc WHERE oid=v_login) IS DISTINCT FROM ARRAY['search_path=public, extensions']::text[]
     OR (SELECT proconfig FROM pg_proc WHERE oid=v_unlock) IS DISTINCT FROM ARRAY['search_path=public, extensions, pg_catalog']::text[] THEN
    RAISE EXCEPTION 'BCRYPT_VERIFY_CAP_POSTCHECK_SEARCH_PATH';
  END IF;
  ...
  IF has_function_privilege('anon', v_bootstrap, 'EXECUTE')
    ...
    RAISE EXCEPTION 'BCRYPT_VERIFY_CAP_POSTCHECK_ACL';
  END IF;
END;
$postcheck$;
