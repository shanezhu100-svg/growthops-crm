-- Emergency rollback for 20260824023041 post_p5_bcrypt_password_byte_cap.
-- WARNING: rolling back re-opens bcrypt silent truncation for passwords beyond byte 72.
...
BEGIN
  ...
  IF v_upsert_md5 <> '941cd0ecb578b212851d818188e3be40' THEN
    RAISE EXCEPTION 'BCRYPT_CAP_ROLLBACK_UPSERT_DRIFT:%', v_upsert_md5;
  END IF;
  IF v_bootstrap_md5 <> 'ee2d5b74bb2b5fa3ed8e1b4bb214da84' THEN
    RAISE EXCEPTION 'BCRYPT_CAP_ROLLBACK_BOOTSTRAP_DRIFT:%', v_bootstrap_md5;
  END IF;
END;
$preflight$;
...
BEGIN
  IF md5(pg_get_functiondef(v_upsert)) <> '48498837112d8cdfa65eb6ecd718d0fe' THEN
    RAISE EXCEPTION 'BCRYPT_CAP_ROLLBACK_POSTCHECK_UPSERT';
  END IF;
  IF md5(pg_get_functiondef(v_bootstrap)) <> 'abc401b6479800280b453b7346f57de6' THEN
    RAISE EXCEPTION 'BCRYPT_CAP_ROLLBACK_POSTCHECK_BOOTSTRAP';
  END IF;
  IF has_function_privilege('anon', v_upsert, 'EXECUTE')
    ...
    RAISE EXCEPTION 'BCRYPT_CAP_ROLLBACK_POSTCHECK_ACL';
  END IF;
END;
$postcheck$;
