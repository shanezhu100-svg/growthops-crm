-- Exact rollback for Production migration 20260824034405 post_p5_user_identity_byte_caps.
-- Removes identity byte CHECKs and restores the immediately preceding function definitions.
...
BEGIN
  ...
  IF v_bootstrap_md5 <> '074db868b0b66ac95fa5ef6bbe5b96c2' THEN
    RAISE EXCEPTION 'IDENTITY_CAP_ROLLBACK_PREFLIGHT_BOOTSTRAP_DRIFT:%', v_bootstrap_md5;
  END IF;
  IF v_upsert_md5 <> '6a6987b7b44cb75ef7a62241cec563c5' THEN
    RAISE EXCEPTION 'IDENTITY_CAP_ROLLBACK_PREFLIGHT_UPSERT_DRIFT:%', v_upsert_md5;
  END IF;
  IF (SELECT count(*) FROM pg_constraint c JOIN pg_class t ON t.oid=c.conrelid JOIN pg_namespace n ON n.oid=t.relnamespace
      WHERE n.nspname='public' AND t.relname='crm_users' AND c.conname IN ('crm_users_name_bytes_check','crm_users_username_bytes_check','crm_users_username_key_bytes_check')) <> 3 THEN
    RAISE EXCEPTION 'IDENTITY_CAP_ROLLBACK_PREFLIGHT_CONSTRAINT_DRIFT';
  END IF;
END;
$preflight$;

ALTER TABLE public.crm_users
  DROP CONSTRAINT crm_users_name_bytes_check,
  DROP CONSTRAINT crm_users_username_bytes_check,
  DROP CONSTRAINT crm_users_username_key_bytes_check;
...
BEGIN
  IF md5(pg_get_functiondef(v_bootstrap)) <> '108be4243b6cd38522a06da77a2ead7b' THEN RAISE EXCEPTION 'IDENTITY_CAP_ROLLBACK_POSTCHECK_BOOTSTRAP_FINGERPRINT'; END IF;
  IF md5(pg_get_functiondef(v_upsert)) <> '941cd0ecb578b212851d818188e3be40' THEN RAISE EXCEPTION 'IDENTITY_CAP_ROLLBACK_POSTCHECK_UPSERT_FINGERPRINT'; END IF;
  IF EXISTS (SELECT 1 FROM pg_constraint c JOIN pg_class t ON t.oid=c.conrelid JOIN pg_namespace n ON n.oid=t.relnamespace WHERE n.nspname='public' AND t.relname='crm_users' AND c.conname IN ('crm_users_name_bytes_check','crm_users_username_bytes_check','crm_users_username_key_bytes_check')) THEN RAISE EXCEPTION 'IDENTITY_CAP_ROLLBACK_POSTCHECK_CONSTRAINTS'; END IF;
  IF has_function_privilege('anon',v_upsert,'EXECUTE') OR has_function_privilege('authenticated',v_upsert,'EXECUTE') OR NOT has_function_privilege('service_role',v_upsert,'EXECUTE') OR has_function_privilege('anon',v_bootstrap,'EXECUTE') OR has_function_privilege('authenticated',v_bootstrap,'EXECUTE') OR NOT has_function_privilege('service_role',v_bootstrap,'EXECUTE') THEN RAISE EXCEPTION 'IDENTITY_CAP_ROLLBACK_POSTCHECK_ACL'; END IF;
END;
$postcheck$;
