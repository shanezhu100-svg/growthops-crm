-- Roll back only the account-correspondence shape change while preserving the
-- current server-identity-only EXECUTE boundary.
...
comment on function public.crm_client_account_safe_summary(text,text) is
  'ADMIN/OPS server-bridge account summary: legacy platform-level FB/TK login identifiers plus Google/Instagram per-account safe summaries; never returns password/2FA values.';
