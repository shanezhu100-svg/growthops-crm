...
# Synthetic-only fixture. It deliberately reproduces the bug boundary: the client
# edit page visibly contains Facebook/TikTok credential labels while a stale
# account-assets aggregate selector remains 0. The active client-form selectedClientId
# must still be used for safe-summary loading. No real credential plaintext exists in
# this test and the password/2FA reveal RPC must never be called.
fixture = r'''<!doctype html>
<html lang="zh-CN">
<head><meta charset="utf-8"><title>credential form regression</title></head>
<body>
  <h1>Synthetic Client</h1>
  <section id="facebook-section">
    <h2>Facebook 资产</h2>
    <div class="account-card" data-account="fb1">
      <div class="field"><label>登录账号</label><input id="fb-login" value=""></div>
      <div class="field"><label>密码 / 2FA</label><input id="fb-secret" value=""></div>
    </div>
  </section>
  <section id="tiktok-section">
    <h2>TikTok 资产</h2>
    <div class="account-card" data-account="tk1">
      <div class="field"><label>登录账号</label><input id="tk-login" value=""></div>
      <div class="field"><label>密码 / 2FA</label><input id="tk-secret" value=""></div>
    </div>
  </section>
  <script>
  ...
  <script>
    setTimeout(()=>{
      ...
      const inputValues=inputIds.map(id=>document.getElementById(id)?.value??'__missing__');
      ...
      const pass=(
        ...
        accountTexts.includes('已保存：fb-login@example.test') &&
        accountTexts.includes('已保存：tk-login@example.test') &&
...
if 'data-credential-regression="pass"' not in dom:
    ...
    for name in (
        ...
        'data-input-values', 'data-summary-client-id', 'data-unexpected-rpc-count',
    ):
...
if '已保存：fb-login@example.test' not in dom or '已保存：tk-login@example.test' not in dom:
    fail('safe login summary text missing from rendered DOM')
...
print(
    ...
    'safe-summary-client=client-1; facebook+tiktok=saved-status+masked-eye; '
    'edit-inputs=blank; reveal-rpc=not-called'
)
