...
// Create success: the tentative client, billing/cost hooks, lead conversion and audit
// reach exactly one shared durable save before any success UI or navigation is replayed.
{
  ...
  eq(successNotice(calls),false,'create success notice held before ACK');
  eq(calls.navigate.length,0,'create navigation held before ACK');
  eq(subject.formDirty,true,'create form remains dirty while ACK is pending');
  const pending=parseState(calls.fetch[0]);
  const created=(pending.clients||[]).find(row=>row.name==='Created Client');ok(created,'pending create state contains client');
  ok(clientRefs(pending.financeReceivables,created.id).length>0,'pending create state contains generated receivable');
  ok(clientRefs(pending.financeCosts,created.id).length>0,'pending create state contains generated cost');
  const pendingLead=byId(pending.leads,'lead-source');eq(pendingLead?.stage,'WON','pending create state converts source lead');eq(String(pendingLead?.convertedClientId),String(created.id),'pending create state links source lead');
  ok((pending.auditLogs||[]).some(row=>row.id==='audit-1'),'pending create state contains attempt audit');
  subject.form.notes='user-continued-edit';subject.formDirty=true;
  resolveFirst(200);await task;
  eq(calls.fetch.length,1,'successful create uses one durable save');
  ok(successNotice(calls),'create success notice emitted after ACK');
  ok(calls.navigate.some(args=>args[0]==='client-detail'),'create navigates after ACK');
  eq(subject.form.notes,'user-continued-edit','form edit made during ACK survives successful create');
  eq(subject.formDirty,true,'form remains dirty when user edits during create ACK');
}
...
// single tentative cloud snapshot.
{
  ...
  const task=subject.saveClient();ok(task&&typeof task.then==='function','edit exposes ACK promise');
  await waitFirstSave(calls);
  eq(successNotice(calls),false,'edit success notice held before ACK');eq(calls.navigate.length,0,'edit navigation held before ACK');
  const pending=parseState(calls.fetch[0]);eq(byId(pending.clients,'client-save-1')?.name,'After Edit','pending edit state contains edited client');
  ok((pending.auditLogs||[]).some(row=>row.id==='audit-1'),'pending edit state contains attempt audit');
  resolveFirst(200);await task;
...
// Failed create rolls back the complete attempt-owned client/lead/finance/selection/
// audit state, persists that rollback, and later persistence cannot resurrect it.
{
  const {subject,calls,lead,initialSelections}=makeRuntime({kind:'create',first:'fail'});
  const task=subject.saveClient();if(task&&typeof task.then==='function')await task;
  ...
  ok(!byId(subject.clients,created.id),'failed create removes attempt client locally');
  eq(lead.stage,'QUALIFIED','failed create restores source lead stage');eq(lead.convertedClientId,null,'failed create restores source lead client pointer');eq(lead.convertedAt,'','failed create restores source lead convertedAt');eq(lead.nextFollowUp,'2026-09-10','failed create restores source lead follow-up');
  eq(clientRefs(subject.financeReceivables,created.id).length,0,'failed create removes attempt receivables');eq(clientRefs(subject.financeCosts,created.id).length,0,'failed create removes attempt costs');
  for(const [key,value] of Object.entries(initialSelections))eq(subject[key],value,`failed create restores ${key}`);
  eq(subject.auditLogs.some(row=>row.id==='audit-1'),false,'failed create removes attempt audit by identity');assertUnrelated(subject,'failed create');
  ok(calls.notify.some(message=>/云端保存失败|未保存/.test(message)),'failed create reports durable failure');eq(calls.navigate.length,0,'failed create does not replay success navigation');
  const rollback=await waitRollback(calls);ok(!byId(rollback.clients,created.id),'rollback cloud excludes failed create client');eq(clientRefs(rollback.financeReceivables,created.id).length,0,'rollback cloud excludes failed create receivables');eq(clientRefs(rollback.financeCosts,created.id).length,0,'rollback cloud excludes failed create costs');eq(byId(rollback.leads,'lead-source')?.stage,'QUALIFIED','rollback cloud restores source lead');eq((rollback.auditLogs||[]).some(row=>row.id==='audit-1'),false,'rollback cloud excludes failed create audit');
...
// the restored pre-edit truth.
{
  ...
  const rollback=await waitRollback(calls);same(byId(rollback.clients,'client-save-1'),before,'failed edit rollback cloud restores original client');eq(clientRefs(rollback.financeReceivables,'client-save-1').length,0,'failed edit rollback cloud removes attempt receivables');eq(clientRefs(rollback.financeCosts,'client-save-1').length,0,'failed edit rollback cloud removes attempt costs');
  const later=await laterPersist(subject,calls);same(byId(later.clients,'client-save-1'),before,'later persist cannot resurrect failed edit');
}

// A failed create may roll back only state still owned by that attempt. Newer same-ID
// replacements, lead edits, finance replacements, selections, form edits and unrelated
// audits created while the ACK is pending must win and must be what rollback persists.
{
  const {subject,calls,lead,resolveFirst}=makeRuntime({kind:'create',first:'deferred'});
  const task=subject.saveClient();await waitFirstSave(calls);
  const attemptClient=subject.clients.find(row=>row.name==='Created Client');ok(attemptClient,'concurrency create has tentative client');const id=attemptClient.id;
  const clientReplacement={...clone(attemptClient),name:'Concurrent Client Replacement',notes:'replacement'};subject.clients.splice(subject.clients.indexOf(attemptClient),1,clientReplacement);
  ...
  eq(byId(subject.clients,id)?.name,'Concurrent Client Replacement','failed create preserves same-ID client replacement');eq(lead.stage,'CONCURRENT_STAGE','failed create preserves concurrent lead stage');eq(lead.convertedClientId,'concurrent-client','failed create preserves concurrent lead pointer');
  if(attemptReceivable)eq(byId(subject.financeReceivables,attemptReceivable.id)?.source,'CONCURRENT_RECEIVABLE','failed create preserves same-ID receivable replacement');if(attemptCost)eq(byId(subject.financeCosts,attemptCost.id)?.source,'CONCURRENT_COST','failed create preserves same-ID cost replacement');
...
{
  const {subject,calls,resolveFirst}=makeRuntime({kind:'edit',first:'deferred'});const task=subject.saveClient();await waitFirstSave(calls);
  const live=byId(subject.clients,'client-save-1');live.name='Concurrent Same-Object Edit';live.monthlyFee=333;subject.form.notes='typing-during-ack';subject.formDirty=true;subject.auditLogs.push({id:'audit-edit-concurrent',action:'CONCURRENT_EDIT'});
  resolveFirst(503);await task;
  ...
  const rollback=await waitRollback(calls);eq(byId(rollback.clients,'client-save-1')?.name,'Concurrent Same-Object Edit','rollback cloud preserves concurrent same-object edit');
}
...
for(const kind of ['create','edit']){
  const {subject,calls,existing,lead}=makeRuntime({kind,withBarrier:false,first:'deferred'});const beforeClient=clone(existing);const beforeLead=clone(lead);
  const result=subject.saveClient();if(result&&typeof result.then==='function')await result;await sleep(20);
  ...
  if(kind==='create'){eq(subject.clients.some(row=>row.name==='Created Client'),false,'create missing barrier removes tentative client');same(byId(subject.leads,'lead-source'),beforeLead,'create missing barrier restores source lead')}
  else same(byId(subject.clients,'client-save-1'),beforeClient,'edit missing barrier restores original client');
}
