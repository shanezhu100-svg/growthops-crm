...
const syntheticTk={
  id:'tk-save-1',name:'Synthetic TikTok',accountId:'tk-account-save-1',
  bcId:'bc-save-1',adAccountId:'ad-save-1',loginAccount:'tk-login-save',
  loginPassword:'',twofa:'',
};
const existing={
...
const target={
  ...
  form:{
    ...existing,name:'After Save',status:'ACTIVE',platform:['TK'],monthlyFee:0,currency:'USD',networkEnvironments:[],
...
const subject=new Proxy(target,{
  get(obj,prop){if(prop in obj)return obj[prop];return undefined;},
...
const saved=subject.clients.find(client=>Array.isArray(client?.tkAccounts)&&client.tkAccounts.some(account=>String(account?.id??'')==='tk-save-1'));
