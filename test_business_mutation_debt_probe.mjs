...
if(start<0||end<0)throw new Error('BUSINESS_MUTATION_DEBT_PROBE_FAILED: EXPECTED inventory block missing');
const block=inventory.slice(start,end);
const names=[...block.matchAll(/'([A-Za-z_$][A-Za-z0-9_$]*)\[[^']+\]'/g)].map(m=>m[1]);
if(names.length!==69)throw new Error(`BUSINESS_MUTATION_DEBT_PROBE_FAILED: expected 69 inventory methods, got ${names.length}`);
const businessFiles=fs.readdirSync(root)
  .filter(name=>/^test_business_.*\.mjs$/.test(name)
    ...
    && name!=='test_business_mutation_debt_probe.mjs')
  .sort();
...
console.log(`BUSINESS_MUTATION_DEBT_PROBE_LIST=${unmentioned.join(',')}`);
console.log(`BUSINESS_MUTATION_DEBT_PROBE_COUNT=${unmentioned.length}`);
