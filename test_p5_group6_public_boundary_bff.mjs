...
for(const invoke of [invokeVercel,invokeCloudflare]){
  // Public status remains reachable with no CRM Session, but browser p_token is
  // never forwarded and the BFF always uses server secret identity upstream.
  ...
  // Login also remains reachable without an existing Session. The upstream
  // token is converted into a secure HttpOnly cookie and removed from JSON.
  ...
  // Invalid credentials remain generic; no Session cookie is issued.
  ...
  // A malformed successful upstream response without a token fails closed.
  ...
  // Both public entry points still require same-origin browser requests before
  // any server-side Supabase call occurs.
  ...
  // Missing server identity fails before upstream for both entry points.
  for(const rpc of ['crm_public_status','crm_login_v3']){
    let upstreamCalls=0;
