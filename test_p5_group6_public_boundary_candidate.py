...
for label, source in (('Vercel', vercel), ('Cloudflare', cloudflare)):
    ...
    require('delete args.p_token' in source, f'{label}: public/login p_token stripping missing')
    require('stripSessionToken' in source, f'{label}: response token stripping helper missing')
...
for expected, message in (
    ('Groups 1–5 are complete', 'completed predecessor chain'),
    ...
    ('No Group 6 forward `REVOKE` migration is included yet.', 'no-forward-migration guard'),
    ('No Group 6 rollback migration is included yet.', 'no-rollback guard'),
...
print(
    ...
    'group5=accepted; production-change=none'
)
