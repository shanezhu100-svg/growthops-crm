...
require(production_ref in workflow and expected_head in workflow,
        'Recovery Bundle v3 must remain pinned to Production ref/head')
...
require("pg_get_userbyid(c.relowner) = 'postgres'" in security_sql and
        ...
        'Existing-object reconciliation must be restricted to postgres-owned objects')
for fragment in (
    ...
    require(fragment in security_sql, f'Post-schema security missing reconciliation control: {fragment}')
...
for source, label in ((full_export, 'FULL_SCHEMA_EXPORT'), (v3_doc, 'RECOVERY_BUNDLE_V3')):
    for fragment in ('Recovery Bundle v3', 'post-schema-security.sql', 'schema.sql', 'event-triggers.sql', 'migration-ledger.sql'):
        require(fragment in source, f'{label} missing v3 recovery authority: {fragment}')
...
start = order_text.find('Required restore order')
ordered = order_text[start:]
positions = [ordered.find('schema.sql'), ordered.find('event-triggers.sql'), ordered.find('post-schema-security.sql'), ordered.find('migration-ledger.sql')]
require(all(p >= 0 for p in positions) and positions == sorted(positions),
        'Recovery v3 authority must document schema -> event triggers -> post-schema security -> migration ledger')
require('has **not** yet been executed successfully' in v3_doc,
        'Recovery v3 authority must not claim the blocked full synthetic acceptance passed')
