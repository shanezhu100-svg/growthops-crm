...
VUE_RUNTIME = (ROOT / 'vue_runtime_only_finalize.py').read_text(encoding='utf-8')
...
for forbidden in ('/api/', '/index.html', '/tailwind.css', '/app/', '/vendor/inter/', '/vendor/fontawesome/', 'vue-3.5.41.renders.js'):
    if any(forbidden in str(r.get('source') or '') for r in static):
        raise SystemExit('STATIC_ASSET_CACHE_POLICY_FAILED mutable/unversioned surface cached immutably: ' + forbidden)

# The catch-all security block must remain first so every static response keeps the
# security policy, while exact cache blocks add only Cache-Control to two assets.
...
print(
    ...
    'html+api+tailwind+app+render-registry+fonts=not-immutable; vercel-cloudflare=parity'
)
