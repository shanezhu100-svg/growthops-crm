...
for marker in ('saveOpeningDeal','saveOpeningProvider','saveAdDataRecord','showOpeningModal','showProviderModal','showAdDataModal','client-form-back','client-form-save','saveClient','stopImmediatePropagation','modalByButton','native-action-bridge-v6','reportValidity','validateButtonForm','finalizeClientNavigation','hardClientReturn','restoreClientReturn','client-detail','growthops_ui_return_client_id','_clientReturn','saveNow','sessionStorage','window.location.replace','window.history.replaceState','clearSessionRestoreCover','getClientRects'):
...
require("vm.currentPage='client-detail'" in bridge,'client save navigation finalizer missing')
require("vm.formDirty!==false||!vm.selectedClientId" in bridge,'client save success guard missing')
require("vm.persist=()=>{persistRequested=true;return true}" in bridge,'client save must suppress delayed duplicate persist before explicit cloud commit')
require("await cloud.saveNow()" in bridge,'client save must await explicit cloud commit')
require("formStillVisible||vm.currentPage!=='client-detail'" in bridge,'client save hard-navigation fallback missing')
