...
for marker in ('saveOpeningDeal','saveOpeningProvider','saveAdDataRecord','showOpeningModal','showProviderModal','showAdDataModal','client-form-back','client-form-save','saveClient','stopImmediatePropagation','modalByButton','native-action-bridge-v8','reportValidity','validateButtonForm','finalizeClientListNavigation','instantNavigate','scrollBehavior','behavior:\'auto\'','clients','client-detail','saveNow','window.history.replaceState','clearSessionRestoreCover'):
...
require("finalizeClientListNavigation=()=>instantNavigate('clients')" in bridge,'client save must return instantly to client list')
require("instantNavigate(vm.form?.id?'client-detail':'clients')" in bridge,'client cancel/back must use instant navigation')
require("vm.navigateTo?.(" not in bridge,'client bridge must not use animated/shared navigateTo path')
require("window.scrollTo({top:0,left:0,behavior:'auto'})" in bridge,'client navigation must explicitly disable smooth scrolling')
