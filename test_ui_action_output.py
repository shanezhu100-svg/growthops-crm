...
for marker in ('saveOpeningDeal','saveOpeningProvider','saveAdDataRecord','showOpeningModal','showProviderModal','showAdDataModal','client-form-back','client-form-save','saveClient','stopImmediatePropagation','modalByButton','native-action-bridge-v11-fast-save','reportValidity','validateButtonForm','finalizeClientListNavigation','quietNavigate','settleScrollTop','scrollingElement','clients','client-detail','saveNow','window.history.replaceState','clearSessionRestoreCover','pendingClientCloudSaves','trackClientCloudSave','beforeunload','正在同步云端','已同步云端'):
...
require('finalizeClientListNavigation();' in client_block,'client save immediate return missing')
...
require(client_block.index('finalizeClientListNavigation();') < client_block.index('trackClientCloudSave(cloud.saveNow())'),'client list return must happen before cloud RPC completion wait')
