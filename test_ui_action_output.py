...
for marker in ('saveOpeningDeal','saveOpeningProvider','saveAdDataRecord','showOpeningModal','showProviderModal','showAdDataModal','client-form-back','client-form-save','saveClient','stopImmediatePropagation','modalByButton','native-action-bridge-v12-balanced-save','reportValidity','validateButtonForm','finalizeClientListNavigation','quietNavigate','navigateWithoutSmooth','settleScrollTop','scrollingElement','SAVE_RETURN_DELAY_MS=350','clients','client-detail','saveNow','window.history.replaceState','clearSessionRestoreCover','pendingClientCloudSaves','trackClientCloudSave','beforeunload','正在同步云端','已同步云端','originalNavigate'):
...
require("navigateWithoutSmooth(vm.form?.id?'client-detail':'clients')" in bridge,'client cancel/back must use reliable navigation without smooth scrolling')
require("if(typeof vm.navigateTo==='function')vm.navigateTo(page);" in bridge,'back/cancel must restore original business navigation')
require("behavior:'auto'" in bridge,'back/cancel navigation must neutralize smooth scroll')
...
quiet_block=bridge.split('const quietNavigate=page=>',1)[1].split('const navigateWithoutSmooth',1)[0]
require('window.scrollTo' not in quiet_block,'quiet navigation must not call window.scrollTo')
require('vm.$forceUpdate' not in quiet_block,'quiet navigation must not force a Vue redraw')
require("scroller.scrollTop=0" in bridge,'client navigation must reset scroll position without smooth animation')
...
require("setTimeout(()=>{" in client_block and 'SAVE_RETURN_DELAY_MS' in client_block,'client save must use a short balanced return delay')
